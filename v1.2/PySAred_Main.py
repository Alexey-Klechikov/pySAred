...
class GUI(PySAred_FrontEnd.Ui_MainWindow):
                    ...
                    for index, th in enumerate(th_motor_data):
                        ...
                        if self.checkBox_SubstrBKG.isChecked() and Qz > skip_BKG and Intens > 0:
                            Intens_bkg = sum(scan_intens[index][
                                             roi_coord[0] - 2 * (roi_coord[1] - roi_coord[0]): roi_coord[0] - (
                                                         roi_coord[1] - roi_coord[0])])
    ...
    def load_detector_images(self):
        ...
        with h5py.File(self.SFM_file, 'r') as file:
            ...
            scan_data = file[list(file.keys())[0]]
            roi_coord = numpy.array(scan_data.get("instrument").get('scalers').get('roi').get("roi"))
            roi_width = int(roi_coord[3]) - int(roi_coord[2])

            self.lineEdit_ROI_x_left.setText(str(roi_coord[2])[:-2])
            self.lineEdit_ROI_x_right.setText(str(roi_coord[3])[:-2])
            self.lineEdit_ROI_BKG_x_left.setText(str(int(roi_coord[2]) - 2 * roi_width))
            self.lineEdit_ROI_BKG_x_right.setText(str(int(roi_coord[2]) - roi_width))
        ...
        with h5py.File(self.SFM_file, 'r') as file:
            ...
            for index, th in enumerate(th_motor_data):
                ...
                if self.current_th == str(round(th, 3)):
                    ...
                    # seems to be a bug in numpy arrays imported from hdf5 files. Problem is solved after I substract ZEROs array with the same dimentions.
    ...
    def load_reflectivity_preview(self):
        ...
        for i in range(0, self.tableWidget_Scans.rowCount()):
            if self.tableWidget_Scans.item(i, 0).text() == self.comboBox_scan.currentText():
                ...
                # ROI region (1400 or 1024 numbers )
                try:
            ...
            for color_index, scan_intens_sfm in enumerate([self.I_uu_sfm, self.I_dd_sfm, self.I_ud_sfm, self.I_du_sfm]):
                ...
                for index, th in enumerate(th_motor_data):
                    ...
                    if not self.checkBox_OverillCorr.isChecked(): overill_corr = 1
                        ...
                        overill_corr, FWHM_proj = self.overillumination_correct_coeff(s1hg, s2hg, round(th, 4), float(
                            self.lineEdit_SampleLength.text()))
                    ...
                    Intens_bkg = sum(scan_intens_sfm[index][roi_coord[0]-2*(roi_coord[1]-roi_coord[0]) : roi_coord[0]-(roi_coord[1]-roi_coord[0])])
                    ...
                    try:
                        ...
                        show_last = len(th_motor_data) - int(self.lineEdit_skip_last_points.text())
                    except:
                        show_first = 0
                    ...
                    if Intens > 0 and index < show_last and index > show_first:
                        ...
                        plot_overillumination.append(overill_corr)
                ...
                if self.checkBox_show_overillumination.isChecked():
                    s3 = pg.ScatterPlotItem(x=plot_angle, y=plot_overillumination, symbol="o", size=2,
    ...
    def draw_2D_map(self):
        ...
        if self.comboBox_polarisation_2d_map.count() == 1: int_detector_image = self.I_uu_sfm
        else:
            if self.comboBox_polarisation_2d_map.currentText() == "uu": int_detector_image = self.I_uu_sfm
            elif self.comboBox_polarisation_2d_map.currentText() == "du": int_detector_image = self.I_du_sfm
            elif self.comboBox_polarisation_2d_map.currentText() == "ud": int_detector_image = self.I_ud_sfm
            elif self.comboBox_polarisation_2d_map.currentText() == "dd": int_detector_image = self.I_dd_sfm

        if int_detector_image == []: return

        int_detector_image = numpy.flip(int_detector_image, axis=0)

        self.graphicsView_map.setImage(int_detector_image, axes={'x': 1, 'y': 0}, levels=(0, 0.1), scale=(1, 15))
        ...
        for i in range(int(int_detector_image.shape[0])):
            spots.append({'y': i*15, 'x': int(self.lineEdit_ROI_x_left.text())})
            spots.append({'y': i*15, 'x': int(self.lineEdit_ROI_x_right.text())})

        self.draw_roi_2D_map = pg.ScatterPlotItem(spots=spots, size=1.5, pen=pg.mkPen(255, 255, 255))
    ...
    def update_slits(self):
        ...
        self.lineEdit_ROI_BKG_x_left.setText(str(int(self.lineEdit_ROI_x_left.text()) - 2 * roi_width))
        self.lineEdit_ROI_BKG_x_right.setText(str(int(self.lineEdit_ROI_x_left.text()) - roi_width))
        self.draw_det_image()

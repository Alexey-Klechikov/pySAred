...
class GUI(PySAred_FrontEnd.Ui_MainWindow):
            ...
            for index, th in enumerate(th_motor_data):
                ...
                if self.current_th == str(round(th, 3)):
                    ...
                    for i in range(int(self.lineEdit_ROI_x_left.text()), int(self.lineEdit_ROI_x_right.text())):
                        spots.append({'x': i, 'y': int(self.lineEdit_ROI_y_bottom.text())})
                        spots.append({'x': i, 'y': int(self.lineEdit_ROI_y_top.text())})
    ...
    def draw_2D_map(self):
        ...
        self.int_detector_image = numpy.flip(self.int_detector_image, axis=0)

        self.graphicsView_map.setImage(self.int_detector_image, axes={'x': 1, 'y': 0}, levels=(0, 0.1), scale=(1, 15))
    ...
    def update_slits(self):
        ...
        self.lineEdit_ROI_BKG_x_left.setText(str(int(self.lineEdit_ROI_BKG_x_right.text()) - 2 * roi_width))

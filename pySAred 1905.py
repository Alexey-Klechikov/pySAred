...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.actionVersion.setText(_translate("MainWindow", "Version 2.2"))
    ...
    def button_Start(self):
        ...
        if self.checkBox_NormDB.isChecked(): DB_info = self.analazeDB()
        else: DB_info = ""
        ...
        for i in range(0, self.tableWidget_Scans.rowCount()):
            ...
            roi_coord = [round(int(self.tableWidget_Scans.item(i, 1).text()[
                                   :self.tableWidget_Scans.item(i, 1).text().rfind(":") - 1]) / 2),
                         round(int(self.tableWidget_Scans.item(i, 1).text()[
                                   self.tableWidget_Scans.item(i, 1).text().rfind(":") + 2:]) / 2)]
            ...
            for scan in scan_data_ponos.get('data'):
                ...
                for index, th in enumerate(th_motor_data):
                    ...
                    if self.checkBox_NormDB.isChecked() and Intens > 0 and len(DB_info) > 0:
                        DB_intens = float(DB_info[str(s1hg) + ";" + str(s2hg)][: DB_info[str(s1hg) + ";" + str(s2hg)].find(";")])
                        DB_err = overill_corr * float(DB_info[str(s1hg) + ";" + str(s2hg)][DB_info[str(s1hg) + ";" + str(s2hg)].find(";") + 1:])
    ...
    def analazeDB(self):
        ...
        if not DB_in_the_list == self.DB_already_analized:
            ...
            for i in range(0, self.listWidget_DB.count()):
                file_db = h5py.File(self.listWidget_DB.item(i).text(), 'r')
                scan_data = file_db[list(file_db.keys())[0]].get("ponos")

                for j in range(0, scan_data.get('instrument').get('motors').get('th').get("value").shape[0]):
                    s1hg = numpy.array(file_db[list(file_db.keys())[0]].get("ponos").get('instrument').get('motors').get('s1hg').get("value"))[j]
                    s2hg = numpy.array(file_db[list(file_db.keys())[0]].get('instrument').get('motors').get('s2hg').get("value"))[j]
                    DB_mon = numpy.array(file_db[list(file_db.keys())[0]].get("ponos").get("control").get("data"))[j]
                    DB_roi = numpy.array(file_db[list(file_db.keys())[0]].get("reflectivity").get("data").get("data"))[j]
                    ...
                    DB_intens = float(db_attenuator) * float(DB_roi) / float(DB_mon)
                    DB_err = DB_intens * numpy.sqrt(1 / float(DB_roi) + 1 / float(DB_mon))

                    slits = str(s1hg) + ";" + str(s2hg)
    ...
    def menu_info(self):
        ...
        msgBox.setText("SuperADAM H5 rapid data extractor V2.2\n\n"
    ...
    def button_load_preview(self):
        ...
        if self.checkBox_NormDB.isChecked() and self.listWidget_DB.count() == 0:
            ...
            return
        elif self.checkBox_NormDB.isChecked(): DB_info = self.analazeDB()
            ...
            for scan in scan_data_ponos.get('data'):
                ...
                for index, th in enumerate(th_motor_data):
                    ...
                    if Intens > 0:
                        ...
                        if Intens > Intens_err:
                            plot_dI_err_bottom.append(math.log10(Intens) - math.log10(Intens - Intens_err))
                        else:
                            plot_dI_err_bottom.append(0)
